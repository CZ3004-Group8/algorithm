    ...
    def bottom_image(self, curr_pos, target_pos, is_start):
        ...
        if not is_start:
            ...
            if offset_pos.x < settings.OBSTACLE_SAFETY_WIDTH:
                ...
                # Move forward 2 * OBSTACLE_SAFETY_WIDTH to clear current obstacle.
                self.commands.append(
        ...
        # We do a forward turn to the right.
        self.commands.append(
            TurnCommand(-math.pi / 2, False).apply_on_pos(curr_pos)
        )
        ...
        # If there is enough y-offset such that we can do a direct forward turn to the left,
